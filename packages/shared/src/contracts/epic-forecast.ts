...
export const EpicForecastTargetSchema = z.object({
  ...
  remainingStoryCount: z.number().int().min(0),
  ...
  manualStoryCount: z.number().int().min(0).nullable(),
...
export const UpsertEpicForecastTargetRequestSchema = z.object({
  ...
  dueDate: LocalDateSchema,
  remainingStoryCount: z.number().int().min(0).max(10000),
  ...
  manualStoryCount: z.number().int().min(0).max(10000).nullable().optional(),
