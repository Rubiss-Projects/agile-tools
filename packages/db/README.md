...
- `DATABASE_URL` must be present in the root environment for Prisma CLI commands
  that connect to the database, such as migrations. `prisma generate` can run
  without a live database URL.
- The `DATABASE_URL_ENV_VAR` indirection (for managed-service deployments that
  inject the PostgreSQL binding under an autogenerated variable name) applies to
  both Prisma CLI config and app/container runtime. The runtime path resolves it
  through `resolveDatabaseUrlFromEnv()` in `@agile-tools/shared`, and the
  production `docker/run.sh` entrypoint exports the resolved `DATABASE_URL` for
  bootstrap-time tools.
